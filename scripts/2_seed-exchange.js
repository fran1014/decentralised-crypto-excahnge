...
async function main() {
  ...
  transaction = await exchange
    .connect(user1)
    .makeOrder(mETH.address, tokens(100), Token.address, tokens(5));
  ...
  transaction = await exchange
    .connect(user1)
    .makeOrder(mETH.address, tokens(100), Token.address, tokens(10));
  result = await transaction.wait();
  console.log(`Made order from ${user1.address}`);
  ...
  transaction = await exchange
    .connect(user1)
    .makeOrder(mETH.address, tokens(200), Token.address, tokens(20));
  result = await transaction.wait();
  console.log(`Made order from ${user1.address}`);
  ...
  for (let i = 1; i <= 10; i++) {
    transaction = await exchange
      .connect(user1)
      .makeOrder(mETH.address, tokens(10 * i), Token.address, tokens(10));
    result = await transaction.wait();
  ...
  for (let i = 1; i <= 10; i++) {
    transaction = await exchange
      .connect(user2)
      .makeOrder(Token.address, tokens(10), mETH.address, tokens(10 * i));
    result = await transaction.wait();
