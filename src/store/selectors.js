...
const decorateOrder = (order, tokens) => {
  ...
  //Note: Token should be considered token0, mETH is considered token1
  //Example: Giving mETH to exchange for Token

  if (order.tokenGive === tokens[1].address) {
    token0Amount = order.amountGive; //The amount of Token we are giving
    token1Amount = order.amountGet; // The amount of mETH we want...
  } else {
    token0Amount = order.amountGet; //The amount of Token we want
    token1Amount = order.amountGive; // The amount of mETH we are giving..
  }

  const precision = 100000;
  let tokenPrice = token1Amount / token0Amount;
  tokenPrice = Math.round(tokenPrice * precision) / 100000;
...
const decorateFilledOrders = (orders, tokens) => {
  //Track prefious order to compare history
